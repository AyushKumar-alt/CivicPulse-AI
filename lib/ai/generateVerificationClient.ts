// Browser-compatible verification client.
// Identical logic to generateVerification.ts but uses btoa instead of Buffer.from
...
function deterministicVerification(input: VerificationInput): VerificationResult {
  ...
  for (const cp of checkpoints) {
    ...
    if (keywords.some(kw => notes.includes(kw))) satisfied.push(cp);
  ...
  if (checkpoints.length === 0) {
    ...
    confidence = Math.round(20 + coverageRatio * 60);
  }

  if (afterImageUrl) confidence = Math.min(confidence + 10, 88);
  if (notes.length < 40) confidence = Math.min(confidence, 45);
  if (iir.severity === "critical") confidence = Math.min(confidence, 68);
  ...
  return { confidence, recommendation, was_issue_addressed, reasoning, concerns, remaining_issues: unsatisfied.length > 0 ? unsatisfied.join("; ") : null };
}

// ── Browser-compatible image loader (uses btoa, not Buffer) ──────────────────

async function fetchImageAsBase64Client(url: string): Promise<{ data: string; mimeType: string } | null> {
  try {
...
export async function generateVerificationClient(input: VerificationInput): Promise<VerificationResult> {
  const apiKey = process.env.NEXT_PUBLIC_GEMINI_API_KEY;
  if (!apiKey) {
    console.warn("[Verification] No NEXT_PUBLIC_GEMINI_API_KEY — using deterministic verifier");
...
Be appropriately skeptical. Vague notes (e.g. "I fixed it", "done") must result in needs_rework or needs_inspection — never approve. Safety issues demand higher confidence before approval. No after-repair image = significant penalty.
...
Return ONLY valid JSON — no markdown, no extra text:
{
  ...
  try {
    ...
    if (hasBeforeImage && input.beforeImageUrl) {
      const imgData = await fetchImageAsBase64Client(input.beforeImageUrl);
      if (imgData) parts.push({ inlineData: imgData });
    }
    if (hasAfterImage && input.afterImageUrl) {
      const imgData = await fetchImageAsBase64Client(input.afterImageUrl);
      if (imgData) parts.push({ inlineData: imgData });
    }
    ...
    if (!text) return deterministicVerification(input);

    const parsed = JSON.parse(text) as VerificationResult;
    if (!parsed.recommendation) return deterministicVerification(input);
