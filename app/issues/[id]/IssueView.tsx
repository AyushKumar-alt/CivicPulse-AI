...
export default function IssueView({ id }: { id: string }) {
  ...
  const ai: any = {
    ...
    summary: rawAi.summary || rawAi.summaryDescription || rawAi.safetyRiskDescription || (issue as any)?.safetyRiskDescription || "Civic issue report analyzed.",
    confidence: typeof rawAi.confidence === "number" ? rawAi.confidence : (typeof rawAi.confidenceScore === "number" ? rawAi.confidenceScore : 0.95),
    ...
    responsible_authority: rawAi.responsible_authority || (issue as any)?.assignedAgencyName || (issue as any)?.assigned_department_name || "Civic Authority",
    ...
    functional_importance: rawAi.functional_importance || (issue as any)?.ai_observations?.functional_importance || "This location is critical for providing essential municipal and utility services to surrounding homes and transit routes.",
    likely_daily_activity: rawAi.likely_daily_activity || (issue as any)?.ai_observations?.likely_daily_activity || "Living, local commuting, pedestrian movement",
    affected_groups: rawAi.affected_groups || (issue as any)?.ai_observations?.affected_groups || ["Residents", "Pedestrians", "Local commuters"],
    estimated_population_impact: rawAi.estimated_population_impact || (issue as any)?.ai_observations?.estimated_population_impact || "~500 residents",
    impact_score: rawAi.impact_score ?? (issue as any)?.ai_observations?.impact_score ?? 8.5,
    impact_reasoning: rawAi.impact_reasoning || (issue as any)?.ai_observations?.impact_reasoning || "High practical impact due to immediate safety risk and service disruption to households.",
    priority_score: rawAi.priority_score ?? (issue as any)?.ai_observations?.priority_score ?? 9.8,
    priority_reasoning: rawAi.priority_reasoning || (issue as any)?.ai_observations?.priority_reasoning || "Critical priority due to immediate public safety hazard and disruption of essential utility services.",
    context_used: rawAi.context_used ?? (issue as any)?.ai_observations?.context_used ?? true,
    context_influence: rawAi.context_influence || (issue as any)?.ai_observations?.context_influence || "high",
  };
