...
export async function POST(
  ...
  // Ensure supervisor actor ID is distinct from submitter ID for independent verification separation (Rule #51)
  const actorId = user.uid.includes("supervisor") ? user.uid : `${user.uid}_supervisor`;
  ...
  try {
    await LifecycleService.close(id, actor);
    return Response.json({ success: true, issueId: id, state: "CLOSED" });
  } catch (err: any) {
