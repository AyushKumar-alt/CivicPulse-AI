...
export async function POST(
  ...
  try {
    await LifecycleService.submitResolution(
      {
        issueId: id,
        afterEvidenceUrl: body.afterEvidenceUrl || body.imageUrl || "https://placeholder.civicpulse.ai/repaired.jpg",
        resolutionNotes: body.resolutionNotes || body.summary || "Field repair work successfully completed.",
      },
      actor
    );

    return Response.json({ success: true, issueId: id, state: "RESOLUTION_SUBMITTED" });
  } catch (err: any) {
