...
export async function POST(
  ...
  if (!body.targetParentId) {
    return Response.json({ error: "Missing required field: targetParentId" }, { status: 400 });
  }

  try {
    const res = await LifecycleService.markDuplicate(
      {
        issueId: id,
    ...
    return Response.json({ success: true, issueId: id, state: "DUPLICATE", rootParentId: res.rootParentId });
  } catch (err: any) {
