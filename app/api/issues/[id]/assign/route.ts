...
export async function POST(
  ...
  try {
    await LifecycleService.fieldAssign(
      {
        issueId: id,
        unitId: body.unitId || "yelahanka_div",
        crewId: body.teamId || body.crewId || "crew_04",
        leadOfficerId: body.officerId || body.leadOfficerId || "officer_101",
        note: body.note,
      },
      actor
    );

    return Response.json({ success: true, issueId: id, state: "FIELD_ASSIGNED" });
  } catch (err: any) {
