...
export async function POST(
  ...
  try {
    await LifecycleService.defer(
      {
        issueId: id,
        reasonCode: body.reasonCode || "OPERATIONAL_DEFERRAL",
        resumeBy: new Date(body.resumeBy || Date.now() + 86400000),
        note: body.note,
      },
      actor
    );

    return Response.json({ success: true, issueId: id, state: "DEFERRED" });
  } catch (err: any) {
